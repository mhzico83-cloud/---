...
const Dashboard: React.FC<DashboardProps> = ({ idps }) => {
  ...
  // توليد بيانات الخريطة مع ربطها بالبحث
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex overflow-x-auto gap-4 pb-4 md:grid md:grid-cols-4 md:overflow-visible no-scrollbar">
        {stats.map((stat, i) => (
          <div key={i} className="glass p-6 rounded-3xl relative overflow-hidden group min-w-[200px] md:min-w-0 flex-shrink-0">
      ...
      <div className="glass p-6 md:p-8 rounded-[3rem] border border-white/5 shadow-2xl relative">
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-6">
          <div className="text-right">
            <h3 className="text-2xl font-black text-white flex items-center gap-3">
              <i className="fas fa-map-location-dot text-accent"></i>
              خريطة التوزيع الميداني للمخيم
            </h3>
            <p className="text-xs text-gray-500 mt-1">تحديد مواقع النازحين والبحث الميداني المباشر</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
            ...
            <div className="relative flex-1 md:w-64">
               <input 
                 ...
                 className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pr-10 pl-4 text-xs text-white outline-none focus:border-accent transition-all"
                 value={mapSearch}
            ...
            <div className="flex flex-wrap gap-4 bg-black/20 p-3 rounded-2xl border border-white/5 shrink-0">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                <span className="text-[10px] text-gray-400 font-bold">إداري</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-cyan-500 rounded-full"></div>
                <span className="text-[10px] text-gray-400 font-bold">نازح</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-white/10 rounded-full border border-white/20"></div>
                <span className="text-[10px] text-gray-400 font-bold">شاغر</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {mapBlocks.map(block => (
            <div 
              ...
              className={`
                ...
                ${!block.isOccupied 
                  ? 'bg-white/5 border-white/5 grayscale opacity-30 hover:opacity-100 hover:bg-white/10' 
                  : block.isAdmin 
                    ? 'bg-yellow-500/20 border-yellow-500/40 text-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.1)]' 
                    : 'bg-cyan-500/20 border-cyan-500/40 text-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.1)]'
                }
                ${block.isOccupied ? 'hover:scale-110 z-10 hover:shadow-2xl' : ''}
                ${block.isHighlighted ? 'animate-flash scale-110 ring-4 ring-white border-white z-20' : ''}
              `}
            >
              ...
              {/* شارة الإداري (Admin Badge) */}
              {block.isAdmin && (
                <div className="absolute -top-1 -right-1 w-4 h-4 bg-yellow-500 rounded-full flex items-center justify-center text-[7px] text-black shadow-lg border border-slate-900">
              ...
              {/* Tooltip on hover */}
              {block.isOccupied && (
                <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-20 border border-white/10 shadow-xl">
                  {block.occupant?.name}
                </div>
              )}
            </div>
      ...
      {/* مودال تفاصيل الشاغل (Gallery Style) */}
      {selectedOccupant && (
        <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col animate-in zoom-in-95">
            ...
            <div className="p-8 pt-2 text-right">
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">الاسم الرباعي للمستفيد</h4>
              <h2 className="text-2xl font-black text-slate-900 mb-6">{selectedOccupant.name}</h2>
              ...
              <button 
                onClick={() => setSelectedOccupant(null)}
                className="w-full mt-8 bg-slate-900 text-white font-black py-4 rounded-2xl hover:bg-indigo-600 transition-all"
              >
                إغلاق النافذة
              </button>
            </div>
      ...
      <style>{`
        ...
        @keyframes flash {
          0%, 100% { filter: brightness(1); box-shadow: 0 0 20px rgba(255,255,255,0.8); }
          50% { filter: brightness(2); box-shadow: 0 0 40px rgba(255,255,255,1); }
        }
        .animate-flash {
